...
function* rootSaga() {
    ...
    yield takeEvery('PUT_MOVIES', editMovieSaga);;
}
...
function* editMovieSaga(action) {
    try {
        const editMovieResponse = yield axios.put('/movies', action.payload);
        yield put({ type: 'EDIT_MOVIE', payload: editMovieResponse.data });
    } catch (error) {
